...
$sql = "SELECT count(*) as nbr FROM `message` WHERE `idU` = ? " ;
...
<?php foreach ($allusers as $alluser): 
	...
	$sql="SELECT * FROM `message` WHERE `idU` = ? AND `idR` = ? " ;
	$listfile = $modal->getAll($sql, array($_POST['idU'],$alluser['id'])) ;	

	if(!empty($listfile))
	{
		?>

		<h5 class="font-weight-bold mt-2">File to : <span class="text-warning"><?= $alluser['name'] ?></span></h5>
		<div class="row">
